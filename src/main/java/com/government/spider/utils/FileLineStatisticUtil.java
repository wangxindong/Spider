...
public class FileLineStatisticUtil {
	...
	public static void outputFileLineStatisticResult(String inputFilePath,
		...
		Set<String> urlSet = statisticMap.keySet();
		// Iterator<String> iter = set.iterator();
		// while (iter.hasNext()) {
		// String key = iter.next();
		// sb.append(key + StaticValue.separator_tab +
		// statisticMap.get(key)+StaticValue.separator_next_line);
		// // System.out.println(key + "--" + map.get(key));
		// }

		for (String url : urlSet) {
			sb.append(url + StaticValue.separator_next_line);
		}
