	...
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
				<!-- 检索  -->
				<form action="quick/list.do" method="POST" name="Form" id="Form">
				<input type="hidden" value="${quickParameter.type}" name="type"/> 
				<input type="hidden" value="${quickParameter.web}" name="web"/> 
				<input type="hidden" value="${quickParameter.keyword}" name="keyword"/> 
				<input type="hidden" value="${quickParameter.module}" name="module"/>
					<table style="margin-top: 5px;">
						<tr>
							<td>
								<div class="nav-search">
									...
									<input autocomplete="off"
										...
										name="title" value="${pd.title}" placeholder="这里输入关键词" />
									</span>
								</div>
							</td>
							...
							<!-- 局选择框 -->
							<c:if test="${quickParameter.type == '7'}">
					...
					<table id="simple-table"
						...
						<thead>
							<tr>
								...
								<c:if test="${quickParameter.type == 8}">
											...
											<tr id="data_main">
											   ...
											   <c:choose>
                                                    <c:when test="${data.readstatus == '1'}">
											             <td id="title_td" class="left"><a style="color:#FF7F00;cursor:pointer;" onclick="godetail('${data.id}','${pd. begintime}','${pd.endtime }','${pd.keyword }','${pd.source }','${data.releasestatus}','${data.readstatus}')">${data.title}</a></td>
											        </c:when>
					                                <c:otherwise>
							...
							<tr>
								...
								<td style="vertical-align: top;">
									<div class="pagination"
										style="float: right; padding-top: 0px; margin-top: 0px;">${quickParameter.page.pageStr}</div>
								</td>
							</tr>
						</table>
					</div>
...
<script type="text/javascript">
    ...
    function goback(){
    	...
    	var currentPage = "${quickParameter.page.currentPage}";
		var showCount = "${quickParameter.page.showCount}";
		var source = '${pd.source}';
		var module = '${quickParameter.module}'
    	window.location.href='<%=basePath%>quick/index.do?title=' + title + '&module=' + module +  '&source=' + source + '&keyword=' + key + '&web=' + web + '&begintime=' + begintime + '&endtime=' + endtime + '&type=' + type + '&currentPage=' + currentPage + '&showCount=' + showCount;
    }
  ...
  //导出excel
    function toExcel(){
	  ...
	  function godetail(id,begintime,endtime,keyword,source,releasestatus,readstatus){
			...
			var source = '${pd.source}';
			var module = '${quickParameter.module}';
		  window.location.href='<%=basePath%>spider/showbyid.do?id=' + id + '&module=' + module +  '&web=' + web +  '&source=' + source +  '&type=' + type + '&title=' + title + '&begintime=' + begintime + '&endtime=' + endtime + '&keyword=' + key + '&currentPage=' + currentPage + '&showCount=' + showCount+ '&releasestatus=' + releasestatus+ '&readstatus=' + readstatus;
	  }
