	...
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
				<!-- 检索  -->
				<form action="spider/province/list.do" method="POST" name="Form" id="Form">
					<input type="hidden" name="is_privince" value="1" />
					<table style="margin-top: 5px;">
						<tr>
							...
							<td style="vertical-align:top;padding-left:2px;width: 100px;">
								 	<select class="chosen-select form-control" name="province" id="province" data-placeholder="请选择地区" style="vertical-align:top;width: 100%;">
									<option value="">--请选择--</option>
									<c:forEach items="${provinceList}" var="provinceList">
										<option value="${provinceList.source}"<c:if test="${pd.province==provinceList.source}">selected</c:if>>${provinceList.source}</option>
									</c:forEach>
								  	</select>
							</td>
					...
					<table id="simple-table"
						...
						<thead>
							<tr>
								...
								<th class="center">来源地区</th>
								<td class="center" >最近查看</td>
											...
											<tr id="data_main">
												...
												<c:choose>
                                                    <c:when test="${data.readstatus == '1' }">
                                                         <td id="title_td" class="left"><a style="color:#FF7F00;cursor:pointer;" onclick="godetail('${data.id}','${pd. begintime}', '${pd.endtime }','${pd.keyword }', '${pd.source }', '${pd.province }', '1','${data.releasestatus}','${data.readstatus}')">${data.title}</a></td>
                                                    </c:when>
                                                    <c:otherwise>
                                                          <td id="title_td" class="left"><a style="cursor:pointer;" onclick="godetail('${data.id}','${pd. begintime}', '${pd.endtime }','${pd.keyword }', '${pd.source }', '${pd.province }', '1','${data.releasestatus}','${data.readstatus}')">${data.title}</a></td>
                                                    </c:otherwise>
                                                </c:choose>
...
<script type="text/javascript">
  ...
  //导出excel
    function toExcel(){
    	...
    	var source = $("#province").val();
    	var currentPage = "${page.currentPage}";
		var showCount = "${page.showCount}";
    	window.location.href='<%=basePath%>spider/excel.do?keyword='+keyword+'&begin='+lastLoginStart+'&end='+lastLoginEnd+'&source='+source + '&currentPage=' + currentPage + '&showCount=' + showCount;
    }
	  ...
	  // 查看详情
	  function godetail(id,begintime,endtime,keyword,source,province,is_province,releasestatus,readstatus){
		  var currentPage = "${page.currentPage}";
		  var showCount = "${page.showCount}";
		  window.location.href='<%=basePath%>spider/showbyid.do?id=' + id + '&begintime=' + begintime + '&endtime=' + endtime + '&keyword=' + keyword + '&source=' + source + '&province=' + province + '&is_province=' + is_province + '&currentPage=' + currentPage + '&showCount=' + showCount+ '&releasestatus=' + releasestatus+ '&readstatus=' + readstatus;
	  }
